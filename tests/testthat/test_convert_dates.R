...
test_that("convert dates converts the correct columns", {
  ...
  expect_true(is.character(df$date) & (is.character(converted_df$date) == FALSE) )
  
  # something_else column is not converted 
  expect_true(is.numeric(df$something_else) & is.numeric(converted_df$something_else) )
