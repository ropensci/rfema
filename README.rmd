...
FEMA data is publicly available at the open [FEMA website](https://www.fema.gov/about/openfema/data-sets), however, bulk downloads are not longer available for most data sets due to the size of the files leaving FEMA's API as the only current option to access some data sets which can be inaccessible for those without prior API experience. This package contains a set of functions that allows users to easily identify and retrieve data from FEMA's API without needing any technical knowledge of APIs. Notably, the FEMA API does not require an API key meaning the package is extremely accessible regardless of if the user has ever interacted with an API. 
