...
## Installation
Right now, the best way to install and use the `rfema` package is by installing directly from GitHub using `devtools::install_github("dylan-turner25", force = TRUE)`. The FEMA API does not require and API key, meaning no further steps need be taken to start using the package
